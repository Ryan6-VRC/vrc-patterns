...
| `maxTextureSize` | `2048` | Clamps the generated face size, so it is the ceiling on VRAM. It does not *set* the faces: Unity reads a 1:1 source as a spheremap and generates faces at half the source width, then clamps here |
| `textureCompression` | `1` (Compressed) | Uncompressed is roughly 4× the bytes for no visible gain on a smooth environment gradient |

As shipped and measured in-Editor, that lands at **`Cube_Glass` 512-px faces, DXT1, 2.00 MB** and **`Cube_Iridescent` 1024-px faces, DXT1, 8.00 MB** — 10 MB of VRAM for the pair, on an entry a consumer drops onto an avatar with a hard budget. Both come from the source images' own dimensions, so **a replacement image's size is what moves this**, and a 2048-px face pair would be 4× it.
