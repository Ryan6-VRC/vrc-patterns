...
- **+1 (mirror clone):** rests on the client's clone semantics (copied parameters, no driver execution). The emulator's MirrorReflection clone does run its animator and so does take this branch, but `IsMirror` is a clip-written AAP and the emulator's AAP readback door attaches to the **local** runtime only — there is no channel that reads a clone's AAP, which is what makes this leg an in-game check rather than the clone being inert (`docs/verify.md` owns the boundary).
