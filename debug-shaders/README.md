...
**Three inherited bugs fixed here**, all of which drew a wrong picture rather than failing:
...
- The normals probe had `GammaToLinearSpace` where upstream has `LinearToGammaSpace` — the conversion inverted, darkening every normal colour non-linearly. The 0..1 remap is meant to be read as a colour, so the gamma direction is what makes the displayed value match the normal it encodes.
...
That 20× ratio is the only handle on the sphere's own size: **the mesh imports at a local radius of 0.003**, so `mesh.bounds` describes the inflation rig rather than anything visible, and `bounds.extents / 20` is how the sphere's radius is recovered. The shipped prefabs scale the body 50× to a 0.15 m presence, and `GammaCrystal`'s `_Core_Radius` is set to the mesh radius itself rather than to a world figure — which is what makes the core silhouette keep tracing the surface at any scale.

None of the three shaders needs this mesh, or any particular mesh. It ships because an icosphere's even topology reads better under a wireframe probe than a UV sphere's pole convergence, and because the bounds rig is worth having by default.
