...
- **`Ladder` ships active, and the ordering that makes that safe is structural, not a race.** Its constraints join the solver on the object's `OnEnable` during instantiation, which necessarily precedes any animator evaluation on that avatar — the components have to exist before anything can animate them. So the off-write cannot land first, whatever frame it lands on, and the controller deliberately carries no delay machinery to protect a margin it does not need. Switching the object off afterwards leaves the constraints joined, which is the whole reason the module can stop costing anything per frame. Shipping it already inactive, or "tidying" it off in edit mode, never joins them and produces a rig that looks identical and pins nothing.
