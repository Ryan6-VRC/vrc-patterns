...
- **Repoint the anchors per avatar.** Slide the `*Offset` GOs until the prop sits right — they are the consumer-editable layer (`AsChildAtRoot` discards edits on the proxy GO itself at build). The mouth offset ships 9 cm forward of the head bone; adjust to the avatar's lips.
