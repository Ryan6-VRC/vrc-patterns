...
`controller.yaml` (FX) and `gesture.yaml` (the Gesture-playable grip half — humanoid muscle curves cannot live in FX) own the state machine, the `AnchorProp/Mode` banding, and every tuned constant. The FX params asset is the single sync surface, and `globalParams` exports only `AnchorProp/Enable`. `Mode` is a plain int written with Set only by the wearer's `localOnly` drivers, so it stays Parameter-Compressor-eligible; the `Enable` menu intent is unsynced and the int carries the outcome.
...
- **The grip seizes only the holding hand** (`Mode` 2 = right, 3 = left); a handoff swaps which hand is seized, and the mouth anchor holds the prop with both hands free and un-seized. Gesture *params* keep firing — only the visible fingers are overridden.
...
- **Arm / disarm thresholds** — proximity `>0` to arm, `<0.00001` to disarm.
