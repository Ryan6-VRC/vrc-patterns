"""Per-frame twin of the compiled palm readout (readout.yaml), evaluated from generate.py's own tree + rung structures.
...
OUT_COLS = ['frame', 'Pattern', 'AxisX', 'AxisY', 'AxisZ', 'S', 'Res', 'MidX', 'MidY', 'MidZ', 'Lever']
def dump(rows, path):
    tw = Twin()
...
def truth(rows, settle=12):
    """Mid and Lever against the CSV's true capsule centre (cx,cy,cz) and axis (ux,uy,uz), tip at the cage origin.
    Lever truth = perpendicular distance from the origin to the axis line through the centre; the gate is its sign vs G.LEVER_T
    once the pipeline has filled (skips the first `settle` frames of each segment)."""
    tw = Twin(); seg = None; since = 0; mid_err = []; lev_ok = 0; lev_n = 0; lev_margin = []
    for r in rows:
        ...
        cu = dot(c, u) / math.sqrt(dot(u, u)); lev = math.sqrt(max(0.0, dot(c, c) - cu * cu))
        want = lev > G.LEVER_T; got = vis[G.P('Lever')] > 0
        lev_n += 1; lev_ok += (want == got); lev_margin.append(lev - G.LEVER_T)
    mid_err.sort()
    return dict(frames=lev_n, mid_err_med=mid_err[len(mid_err) // 2], mid_err_max=mid_err[-1], lever_sign_agree=lev_ok / lev_n,
                lever_truth_min=min(lev_margin) + G.LEVER_T, lever_truth_max=max(lev_margin) + G.LEVER_T)
