...
**Capture is relative, by disable-hold.** The prop never snaps to the hand: on grab it holds the pose it had, a `Held` node under the recovered frame is driven onto the prop's rotation, and once the readout pipeline has primed `Held`'s constraint is disabled, so it keeps its local pose under the frame and rides it; the capture is re-taken the frame the readout settles, and `Rotor` follows `Held` throughout. `Container/Damped` is a self-sourcing rotation damper (`spring-damping`'s pattern) chasing `Rotor`, and the payload rides it: a few frames of lag that smooth the readout's pattern hops and, on remotes, the stepped hand. It sits beside `Rotor`, never under it: a child's world rotation follows its parent before the self-source reads it, and the damper then has nothing to lag. Release disables `Rotor`'s constraint, which freezes the rotation the same way `grab-prop` freezes position. Every client captures on its own readout, so the offset each client holds differs by however far the hand turned during that client's acquisition; the fixed dwell before capture keeps that to the IK delay.

## How it works

The glue controller is `grab-prop`'s cell, clip table replicated binding for binding, plus four states around the latch. `Acquire` is entered on the grab: the cage sits at its acquisition scale with filters open, and the palm arriving inside all eight boxes is what advances to `Latched`, which shuts the filters on frame 0 (what is inside is what stays latched) and expands the hosts to working scale on frame 1. `Settling` holds the prop frozen for a short fill while the readout pipeline primes; `Following` then lets the prop ride the still-settling frame for the rest of the capture dwell, so rotation moves early and a wrong-line hold shows as a brief jump rather than a dead interval; `Settled` polls every frame for the residual to fall and the half-length to read palm-plausible, exits to `Held6` or `Held5` by the lever gate, or back to `Acquire` at the timeout, reopening the filters. That loop is the hand-identification mechanism, and it is a residual test, not an identity test: a second palm latched with the grabbing one usually keeps the residual high, the loop reopens, a broken latched contact cannot re-latch, and the readout converges on the palm that stays with the tip. One case the reopen does not cover: a latched palm that leaves a box while the filters are shut and is back inside before the timeout is not acquired when the filters reopen either (acquisition is an enter event, measured in the emulator), so that hand stays position-only for the rest of the grab; `absolute-grip-prop` adds a receiver stow for it. A two-handed grab loops indefinitely and the prop carries position-only. Each box reads its nearest sender, so two palms whose union happens to read like one plausible capsule pass the settle gate and the prop captures a blend of the two; the half-length band is the only guard, and narrowing it toward the surveyed hands rejects more such unions at the cost of large hands.
...
| Dwells | `SETTLE_FILL`, `CAPTURE_DWELL`, `SETTLE_TIMEOUT`, `DISABLED_DWELL` | seconds frozen while the pipeline primes; seconds before a capture may fire; seconds before the latch loop reopens; seconds a receiver stays off |
| Damping | prefab `Container/Damped` target weight | fraction of the way the payload closes on `Rotor` per frame; lower is smoother and lags more |
