...
def emit_sweep_layer(o, c, ks):
    ...
    # gate_base × One beside gate_front × Sweep, the slot boxes' own pair (sweep_cfg + front_scale): a transform curve
    ...
    hold_tree("Sweep wait", [("sw_sweeping", f"{P}/One"), ("sw_hold_sweep", f"{P}/Sweep"), ("sw_latch_base", f"{P}/Sweep"), ("sw_hold_silent", f"{P}/Silent"), ("gate_base", f"{P}/One"), ("gate_front", f"{P}/Sweep")])
    o("        transitions:")
    o(paused)
    o(off)
    ...
    hold_tree("Sweep ramp", [("sw_ramp", f"{P}/One"), ("sw_hold_sweep", f"{P}/SweepBase"), ("sw_hold_base", f"{P}/SweepBase"), ("sw_hold_silent", f"{P}/Silent"), ("gate_base", f"{P}/One"), ("gate_front", f"{P}/Sweep")])   # the gate reads last frame's Sweep: one frame behind the front, like a slot's own Sweep state
    o("        transitions:")
    o(paused)
    o(off)
...
def emit_sweep_clips(o, c):
    ...
    clip("gate_base", {f"{GATE}/Transform.m_LocalScale.{ax}": fmt(collapsed) for ax in ("x", "y", "z")},
