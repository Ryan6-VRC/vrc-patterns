...
So each slot is one layer: Disabled → Armed (flag shut; everything inside stays invisible) → Open (flag up; exactly one slot at a time; a hand read by only some of the three boxes parks the slot in Partial for a step and then Recycles it, since an Open slot that stalls blocks every admission) → Latch (flag shut and cube expanded in one write) → TrackOut / TrackIn (the readout tree, burst off / on) → Recycle (collapse a step, restore a step) → Armed. The ring rule opens the next Armed slot in the same evaluation the Open slot latches; when nothing is Open, the lowest Armed slot opens itself on its clip's exit-time crossing, quantized so the flags it reads are fresh. The burst is arithmetic, not a receiver: the tree sums `x = 2h·X+ − h − r` per axis into the AAPs and `Output`'s localPosition, three 1D lookups add x², y², z² into r², and the transition compares r² against the two radii — which is what makes the zone a sphere and the hysteresis per-sender.
...
- **3K coincident receivers on every copy.** Two wearers side by side at K=4 sit near the ~24-receiver cluster bug; K is the knob and off stows them.
