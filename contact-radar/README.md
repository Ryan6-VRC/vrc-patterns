...
- **Exhaustion.** With every slot latched, a new hand is rejected by every slot until it leaves the cube and re-enters, and a slot that frees later does not re-examine it. Exhaustion during a sweep leaves the sweep flag up; the next recycled slot resumes the front from where it stopped and finishes, so a resident the front had not reached fires the payload when it is finally admitted, and a hand that stopped inside the frozen front is never re-examined.
...
- **Every hand already inside fires the payload again at each load, late join, mirror and distance-hide resume.** The sweep that re-acquires them cannot tell a resident from an arrival, and the rig carries no quiet endpoint, so an edge reader (the shipped burst, a ping) fires once per resident at each of those events while a level reader (a marker, a mosaic) simply comes back on. Distance-hide resumes are frequent in a busy instance; a consumer whose edge payload must not repeat needs its own memory of who was already there.
