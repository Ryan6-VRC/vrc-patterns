...
1. The index is a batch COUNTER spanning `indexLoops` full loops, not a
   position-in-cycle. A receiver resumed from a culling pause advances only if
   the counter aliased across the whole pause — probability ~1/(indexLoops *
   batchCount) per resume instead of ~1/batchCount. A finite counter cannot
   detect an unbounded pause deterministically; width buys probability, and
   the residual is declared, not hidden.
...
def main():
    ...
    cycle_s = batch_count * (BATCH_SECONDS + 1 / 60 * 0.5)
    ...
    o(f"# = {wire_bits} synced bits carrying {payload_bits} bits of words ({len(numbers)} bytes + {len(bools)} bools)")
    ...
    for b in bools:
        o(f"  {b}: bool")
    o(f"  {p}/Cycle: int             # remote freshness: +1 per applied cycle")
    ...
    print(f"wrote {out}: atomic={atomic}, {batch_count} batches x {loops} loops (period {period}), "
          f"{wire_bits} wire bits, {payload_bits} payload bits, ~{cycle_s:.2f}s cycle, "
          f"alias P~1/{period}")
