...
**Which edge of the cell's cycle goes stale is authored by the hierarchy, not rolled by the solver.** When one VRC constraint's driven target transform is a hierarchy ancestor of another's, the ancestor-target constraint solves in the strictly earlier execution group, so across a mutual source dependency the ancestor side reads the descendant side exactly one frame stale — measured black-box on isolated pairs, 15/15 invariant with inverted nesting flipping the stale edge 5/5, while flat mutual pairs rolled 5/15-vs-10/15 in the same scenes, including flipping between a fresh build and a reopen from disk off byte-identical content. `SourcePosition` is therefore a **child of `Container`** — and it is `Container`'s own constraint that reads it, so `Container` solves first, reads the previous frame's sample, and the capture holds. The relation outranks everything registration-dependent — no source-list addition, duplicate-swap, component or sibling reshuffle, prefab round-trip, scene reload, or scale change can flip *this* pair (11-entry adversarial gauntlet on this rig, 11/11 stale-edge invariant, capture error 0.000000 m throughout, operator-verified by grabbing).

**A source-list edit still rewires every *other* pair's order, without touching a clip.** Solve order derives from the source/target transform graph and never consults weights, so *adding* a source to a cell constraint — `GrabPosition` most temptingly, to park the chain's rest on a synced or displayed frame — reorders evaluation even at weight 0, and on a pair with no hierarchy relation between its targets an added source on another constraint's driven target flipped the release capture to home twice (frame-measured, two different targets; carry unaffected, compiled clips identical), while a source with no constrained ancestor was inert — "no constrained ancestor" counts the node's **own** constraint too, so a source that carries one is never in the inert class. The sanctioned move is to *repoint* the existing `HomeAnchor/Offset` slot (`source0` on `GrabPosition`) at a plain, constraint-free child transform of the frame the rest should ride — two sources always, the shape the ancestor production rig ships — measured correct in emulator and client. When a value-identical constraint edit changes behavior anyway, read the **frame lag** across the cell's contested edge rather than the constraints' group indices: `Container` reading `Container/SourcePosition` one frame late is the hold working, and same-frame is the break. The index is a depth and does not decide the cut — it has mis-called this very cell while it behaved correctly (`Container` group 1/3, `SourcePosition` group 4/6 on the working rig). §Verifying the install below owns the lag method and the numbers.
...
**Verify the cell's stale edge by frame lag, never by group index.** The capture rests on `Container` reading `SourcePosition` exactly one frame stale (§How it works); confirm that lag directly rather than reading `LatestValidExecutionGroupIndex`, which is a longest-path **depth**, not the cut, and disagrees with behaviour — measured across six cells on one avatar against behaviour confirmed by grabbing, an index comparison predicted 5/6 and the relative frame lag 6/6, the miss a cell holding an inverted index while behaving correctly.

The method: in play, past frame 1 (`LatestValidExecutionGroupIndex` serializes and retains its last valid value, so an edit-mode or entry-frame read reports the previous session and looks plausible), ramp a transform upstream of the cycle a fixed step per frame and record how many frames behind each node lands. Lag above the cycle is common-mode and cancels, so only the step across the contested edge counts, and the node reading across the stale edge lands exactly one frame further back than its own source. Lag is in frames, so halving the ramp rate must not change the reading — that invariance is what separates it from an artifact of the drive. Re-verify after any change to the avatar's constraint graph; a new edge inside the cycle is what moves a cut. The lag read predicts local behaviour and has been measured to track it; anything a **remote** observer sees still needs two clients in-game.
...
The prefab is the shipped artifact and ships no builder — edit it in place. `Locked` on every constraint, source weights swapped by the clips.
    ...
    │  └─ SourcePosition  (0, 0, 0)   VRCPositionConstraint follows DropPosition (sample-and-hold cell) —
    │                                 the nesting under Container is load-bearing: it authors the solve order
    │                                 the release capture rides (§How it works). Never flatten it.
