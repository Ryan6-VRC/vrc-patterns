...
- **This entry is generated output.** `controller.yaml` and `built/manifest.json` are written by `generate.py` from its CONFIG — edit the config, rerun, recompile `built/`; never hand-edit either file committed here — check their freshness by regenerating and reading git diff, then run `generate.py --check`. `build(config)` is the fragment-mode door for a generator composing these channels into its own controller — the contract is in `generate.py`'s docstring.
...
`generate.py`'s docstring is the design record and owns the whole mechanism: the two `IsLocal`-switched states, the decode `x̂ = ±k/(2ⁿ−1)`, the 1D-on-keep smoother tree and its ±1 thresholds, why frametime compensation is backward Euler rather than either of `../smooth-frametime`'s two flavours (that entry is the repo's smoother catalog; lift its `remap` instead when one hand-tuned channel needs exact independence over a known band), and why the frametime rig is duplicated per compensating state.

Two consequences of that shape it does not carry. **±0 decodes to 0 structurally**, so a sender that asserts `Negative` at zero magnitude is harmless. And the one-frame garbage `dt` at avatar load lands as a single frozen frame — keep clamps to 1 — then self-clears.
