...
| Crawl gain | **static**, default **g = 0.5** (`VRCPositionConstraint` source0=Output=1 : source1=self=1; g = w₀/(w₀+w₁)) | closes ~50 %/frame — locks on in a few frames, faster than the 6-contact `contact-tracker` and its ~1 s acquisition settle-dwell (the exact readout needs none). **Tunable, both ways and live:** no clip animates the weights (WD-on holds the prefab value), so scrub source0 in the inspector during play — lower for a softer, floatier follow, higher (→ g=1) for a locked-on snap. Self-weight is the brake; first-order stable for g<1, framerate-dependent by design |
