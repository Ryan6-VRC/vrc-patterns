...
- **Params:** `ContactTracker/Enable` (bool, in) — synced, unsaved; off is the reset (recalls the cage to `HomeAnchor/Offset`). `HomeAnchor` is an MA BoneProxy (Hips, AsChildAtRoot); retarget the proxy or adjust its `Offset` child to move home. Anchoring by BoneProxy is safe here **only because** no clip path runs through `HomeAnchor` — it is referenced purely as a constraint source, which survives the build-time reparent. The six `ContactTracker/{X,Y,Z}±` floats are sensing — never synced, never menu-exposed.
