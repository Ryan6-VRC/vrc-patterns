...
**The release choreography, per client.** On a witnessed `_IsGrabbed` falling edge, the released clip freezes **`Damped`** — the display leaf — for its 0.5 s, which is grab-prop's Container freeze instanced here: the release frame poisons the sample cell itself (the tip snaps to its rest and `SourcePosition` follows before any same-frame write protects it — grab-prop's README §How it works owns the trap), and the one node measured holding the last good value in that frame is the leaf across the constraint cycle's stale edge. The root then re-anchors onto the frozen drop, the tip settles there, and `SourcePosition`'s off→on→off pulse re-samples it — healing `Carry` and the measurement inside the clip (measured: poisoned six frames, healed three frames before Placed re-enables `Damped` onto an already-agreeing chain; the drag heading spikes 180° under the frozen display and settles back exactly). The wearer stamps `Placed<N>` (localOnly driver; the value syncs), and the ~0.3 s poisoned measurement window disappears under the Bridge timer's full-ring wait. A remote swaps to the reconstruction only after the Bridge timer — sized in `generate.py` as full measure ring + full wire loop + fine-stage re-lock + buffer, because until a post-release commit has crossed the wire, the word table still names a mid-carry pose. The swap is a damped glide over the remote-vs-wearer drop disagreement (IK-delay-sized), so the timer's exactness is not load-bearing. Every other placement state is entered by predicate on current values; Bridge is the sole edge-entered state, so no reset, cull resume, or missed edge can wedge a remote.
