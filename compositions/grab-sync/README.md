...
**The chord law.** On the wearer no weight on the display path ever animates — IsLocal chords bind exactly grab-prop's clip table (mapped: vanilla `Container` → `LocalPose`), and the wearer rides `Container ≡ LocalPose` through WD-ON defaults. Remote chords own the `Container` weights, and the only transitions that move them are the bridge→placed crossfade and the carried entry, both at least one animator frame from any physbone edge. This is what keeps grab-prop's release-frame race unreachable at every node; `generate.py --check` asserts both halves plus the mapped binding-superset against `grab-prop/controller.yaml`.
