...
**Known residual — remote re-grab heading.** At the remote carried entry the `Container` rotation swaps word→drag in one frame, and the drag bone's heading at that moment is its own trailing state — near the word for a remote that watched the carry, arbitrary after a fresh join or cull resume — so the swap can show a one-time snap that self-corrects within a few frames of actual hand motion. Do not try to fix this by parking the drag bone: the physbone's sim state is a world-space trail particle nothing animatable reaches — a constraint on the bone only overwrites its transform *after* the sim (release resumes from the particle, snapping anyway), and cycling the physbone's `m_Enabled` resets the chain to authored rest regardless of `resetWhenDisabled` (all three park mechanisms measured dead in play, 2026-08-20). The one available softener is a short rotation-weight crossfade at the carried entry, which glides into the stale-then-self-correcting heading instead of cutting to it.
