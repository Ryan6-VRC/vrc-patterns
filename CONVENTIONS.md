...
**Two readers, one document, ordered by depth** (the `write-for-agents` skill's dual register). The top is the shape VRLabs' module READMEs use, in plain technical writing a person can follow cold: a one-line description, a short `How it works` of terse mechanism bullets, an `Install guide` of numbered steps, `How to use`, `Additional notes`, and a `Performance stats` block of counts. Short sentences, one idea each, no history. Below that, the depth sections: `Knobs`, the provenance line, `Design notes` (which names where the state-by-state walk lives, the generator's docstring where one exists, and states only the invariants and the reasons), `Traps`, what is unproven, install discriminators, and the `Rig` tree. A fact appears once, at the shallowest depth its reader needs it. `contact-radar/README.md` is the worked example of the shape. The process that produces one, audits, draft, review loop and PR, is the metarepo's `write-pattern-readme` skill.
...
`tools/gate.ps1` runs it per entry at any depth (`built/`/`assets/` never descended), plus structural comparison of the emitted menu and params asset, plus a prefab-integrity pass failing four things no reviewer can see in a committed artifact: a missing MonoBehaviour script, an anchor seam (`nondestructive.md` rule 1; `CheckAvatar` names it), a committed consuming-project path, and a constraint source slot past its list length (`runtime.md` §Constraints; `ReportGimmick.ScanConstraintLengths` names it, and only reports it on a composed avatar — an entry here is ours, so the gate rules it FAIL). That pass counts committed prefabs, not entries. A model-importer pass then reads every committed model `.meta` and fails Read/Write off or blendshape normals set to Calculate without legacy: the SDK panel refuses an upload over either on any avatar carrying the mesh, and no consuming venue can correct it, because a VPM resolve rewrites its `Packages/`. The gate does not round-trip the schema — that is `avatar-tools`' own fixpoint property, and a break there must not fail an entry.

**Regenerate `built/` as a unit** — controller + params + menu, over the committed `.meta`s so GUIDs hold. If the emitted parameter list moved without a deliberate YAML edit (a `scratch:` flip, the compiler's reserved set), settle that first — regenerating blind commits it as though it had been reviewed. The procedure is a `CompileController` run in any venue that `file:`-mounts this library, with both the source and the outDir addressed as `Packages/com.ryan6vrc.patterns/…` — Unity resolves those to the checkout, so the emit reads the entry's yaml and writes its `built/` into the working tree over already-imported `.meta`s, which is what holds the GUIDs. An absolute source path compiles and holds them too, so `gate.ps1`'s out-of-project run is equivalent here. Each regenerated `.meta` moves its `srchash:` line — that is the stamp tracking the source it was compiled from, not drift — and any `compiled-from:` path a `.meta` still carries empties, because the compiler records none; both are the stamp, and a regeneration that changed nothing else moves exactly these.
