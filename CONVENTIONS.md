...
**Keep the relation, route the value.** A row names where the knob is authored (the `controller.yaml` clip, the prefab field), what turning it does and which way, and the relation that makes it legible (`g = w₀/(w₀+w₁)`; zone side = 3 × scale) — but not the tuned number, which lives once at the authoring site so a retune touches one file. A second entry cloning the rig physically copies the value (a prefab float cannot reference another entry), so the canon's constants block **names its copy sites** — the route must run both ways, since a retune starts at the canon where an inbound-only link is invisible. Three things stay quoted: a value the rig *produces* (a latch envelope, a worst-case error, a Pattern's Behavior row), which has no other home; a structural constant the design rests on (synced-bit count, cage geometry, readout coefficients), a managed echo naming its source because re-deriving it *is* the design; and a `Rig` section, the declared spec its hand-maintained prefab is kept against.
...
- **Seam** — which framework merges it (MA `MergeAnimator` vs VRCFury `FullController`), the anchor, and the **binding frame the merge resolves** (MA `basis:` ↔ `pathMode`; VRCF per-binding, `basis: mount-root` ↔ `rootBindingsApplyToAvatar: 0`). CompileController is frame-blind, so this is load-bearing — record it. A module's rest geometry (home, park, deploy point) **ships anchored to the avatar** — an Anchor GO (MA `BoneProxy`, AsChildAtRoot) with an `Offset` child as the referenced target; only object-referenced, never path-animated, nodes may be proxied, and the gate fails a violation under a VRCFury merge. Unanchored rest geometry loads at the avatar-root origin — the wearer's feet. (`nondestructive.md` owns the frame and build-order mechanics; `gimmicks.md` §Packaging owns the anchor idiom.)
...
**A bare `Toggle` therefore needs one control *and* a module that cannot be instanced twice** — two instances export the same un-prefixed name and collide. Single-instancing rules out only the module colliding with *itself*; the host-capture exposure above is unaffected. `head-deform` is the sanctioned case (one control, one head per avatar), and its two prefab variants may both export `HeadDeform/Active` because they are alternatives, never composed together.
