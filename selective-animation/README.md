...
- **Required assets:** `assets/SelectiveAnimationBeam.mat` (Unity Standard, self-contained — no Poiyomi or lilToon dependency) and `assets/SelectiveAnimationMenu.asset`. The material carries the beam's colour; nothing animates it. `Payload` is a placeholder sphere on the built-in default material.
...
**The one check is the beam under the held button, and it must be done in play mode.** Hold `Tag`: the beam appears on your own copy, not on a remote clone, tracks the world ray as you walk it to a wall, and vanishes on release — and `Aim` must have landed under the right hand, which only happens at build. If the beam never appears, or appears and never goes away, the merged clips lost their bindings: check that nothing above an animated node carries a non-VRCFury anchor (**Interface**) — that failure is silent, with no error.
...
## Rig
    ...
    │                               VRCFury's and not MA's — see Interface
