...
|---|---|---|
| Released pulse phases | the `released` clip's phase keys — freeze at t=0, then a re-sample window, then hold | `grab-prop`'s rig and its emulator sweep own the timings; the pulse length is what the remote settle below is defined against |
| Remote release-settle | not its own constant — **= the `released` pulse length**, since remotes route by pair at pulse end | covering the sync tick is the point: a world drop must not flash through a stale mode. Measured on the source avatar; the in-game batch owns it |
| Remote boot dwell | the `timer` clip's length | long enough for the pair to arrive before a late joiner poses off it — `grab-prop` in-game candidate |
| Head-catch column | prefab `TrackingPoints` rest `localScale` + the scale constraint's `ScaleAtRest`, edited together — zone side = 3 × scale **per axis** | taller than wide, so lowering onto a head catches down the whole column; doubles as the release-arbitration zone. In-game wear-test owns the dimensions. A non-uniform column costs one latch frame of radius-compensation skew (`box-tracker`) |
| Tracking geometry / crawl gain | ×1 absolute boxes constraint-pinned to `World.prefab`; gain per `box-tracker`'s crawl-gain row (prefab `source0` + the `tracked` clip's curve) | `box-tracker` owns both — exact readout, no convergence dwell, and an acquisition ease-in from g=0 (a clean freeze) damping the first-frame gap. The cage geometry is absolute so avatar scale never skews the readout |
