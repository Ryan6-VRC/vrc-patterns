...
Measures what the shipping client actually delivers of another player's synced parameters — schedule, loss, downshift, pauses, snapshot coherence — with no OSC monitoring of any remote client: the wearer's remote copy relays what it received onto world-anchored contacts, and the observing client's own receivers mirror that into its OSC surface. Two clients each wear one copy and measure each other symmetrically; the packaged novelty is the analog contact relay that gets a remote copy's received bytes out same-frame, for **49 synced bits** (four rate-ladder counters, a torn-snapshot pair, one reset bool). An instrument, not a capability: it exists to settle `word-channel`'s `BATCH_SECONDS`, the VRCFury extra-frame guard, distance downshift, and snapshot coherence, each with a measured in-game number read off the log.

## Provenance

The rate-ladder senders are `word-channel`'s Send/Extra cadence idiom (itself the VRCFury Parameter Compressor's, source-studied), carried here as a **managed echo, not a call**: `build(config)` refuses a single-word table by design, and an instrument wants the idiom pinned as measured — a later change to `word-channel`'s guard deliberately does not propagate into this probe. The bare 0.1 s rung strips the guard hop that idiom exists for — the experimental deviation this probe tests, with `word-channel` itself untouched. The mirror gate is `mirror-detect`'s driver race applied to the rig's GameObject. The world park, face-mode analog readout, edge-guard discipline, and calibration approach descend from `object-sync`'s measured rig; the frametime rig is lifted from `blendtree-math`. VRLabs Custom-Object-Sync (MIT, © VRLabs) is those entries' studied ancestor and is credited through them.

## Interface

- **Params in**: `SyncProbe/Reset` (bool, synced, unsaved) — the only control: while true, tallies clear on every client and stride baselines re-stage. The menu toggle drives it; an OSC writer may drive the bare name directly. **Hold it on ~2 s before releasing**: the clear runs on the *remote* copies, and a synced set→clear pair under ~0.2 s is never seen remotely at all (`docs/runtime.md` §Parameters) — a quick flick clears one client's tallies and not the other's, which reads as a measurement result.
...
- **Menu**: one `Reset` toggle in a `SyncProbe` submenu, merged from the built menu asset.
...
- **Reset before every measurement segment — on, hold ~2 s, off.** A fresh observer's first delivery lands as one spurious `Big` per rung (stride baseline acquisition); reset clears it, and the level-held reset also re-stages baselines so releasing it never mints another.

## How it works

**Senders (wearer's client only).** Each rung is a state loop publishing a local float counter to its synced Int and then incrementing, as two driver behaviours in list order — a driver `Add` clips a synced Int at 255, so wrap is an explicit state that publishes 0 by `Set` and re-seeds the counter. The three guarded rungs dwell `exitTime` seconds then take one conditional-hop frame (the compressor's guard); the bare rung is a pure exit-time flip-flop. On every rung the wrap state is itself a tick — 0 holds a full cadence before the next value — because a wrap that hopped straight on held 0 for one render frame, under the ~0.1 s tick, minting a phantom stride-2 on the guarded rungs alone: a bias injected into exactly the guarded-vs-bare comparison the ladder exists to make. The torn pair rides two complementary counters written by one driver each 0.1 s tick — a `convertRange` inversion was measured off by one (205 → 49; the float remap truncates on the Int copy), so complement-by-construction replaced it. `FpsBand` is a threshold ladder over the frametime rig, giving the logger this client's own frame clock as a declared Int.
...
**Tallies (the on-board cross-check).** Per rung, a Direct subtree computes `Delta = Cur − Prev` and a transition ladder buckets each change as stride 1 / 2 / 3 / 4–15 / big-jump (≥16), with the mod-256 wrap ranges as explicit OR rungs; a driver adds the tally (floats — unsynced, never clipped) and re-stages `Prev`. The torn tally counts a `TornA + TornB ≠ 255` persisting ≥3 frames (a real torn tick persists ~a full tick; a render-frame boundary does not). Tallies exist to catch a lying relay: if post-hoc stride counts from the live streams disagree with them, the relay lied. Three declared bounds: the 8-bit counters alias at 256 ticks (12.8 / 25.6 / 51.2 s per cadence), so a pause near a multiple tallies as a small stride and one of exactly the period tallies as nothing — the timestamped live streams disambiguate, tallies alone cannot; classification takes ~5 frames, so the 0.05 s rung's tally is **advisory** (its truth is the live relay stream, which has full per-frame throughput); and a pause's resume can occasionally land one rung's jump in S4 instead of Big (observed once in emulator testing, mechanism unpinned) — read S4+Big jointly as "non-small stride" when cross-checking pauses.

**Scan bus.** A cycler round-robins 22 registers (5 buckets × 4 rungs, torn count, and a `SweepMark` sentinel — Sel 21, Val 128 — that keeps the count even so the strobe has an edge at the sweep wrap) onto three analog lines: **value = tally / 10** (`convertRange` 0..2550 → 0..255; a raw copy saturated inside one sweep on every rung faster than 0.2 s), select index, and a strobe that flips as each register latches, so a logger trusts value/select only between strobe edges. One full sweep is 33 s at the 1.5 s dwell; the fastest register (R05's S1) lasts ~128 s before its ceiling, two clean sweeps. An over-ceiling register saturates at 255 — a driver `convertRange` copy clamps an out-of-range source (measured: a tally seeded to 9999 read back exactly 255) — so a `ScanVal` mirror of exactly 0 means relay collapse and nothing else; an empty tally reads the rest offset (p ≈ 0.046).

**Heartbeat, the anomaly classifier.** A two-state flip-flop of complementary GO-active clips — pure local time, zero network dependence — keeps exactly one of two Constant senders active. At the observer: heartbeat clean + counters skipping = genuine sync loss; both mirrors frozen with exactly one high = animator pause, not loss (emulator-verified under an induced pause); both zero = relay collapse (anchor broke); erratic = contact trouble, discard the interval. **A live heartbeat with an analog mirror stuck exactly at 0 is a fifth state, distinct from relay collapse: an uninitialized relay** — the remote probe copy instantiated during the observer's own world load, whose senders are authored out-of-box and animated in by the relay layer, never started (in-client only, unreproduced in the emulator). Cycling the wearer out/back once heals it immediately; treat it as an init check before the liveness cross-check, not as loss.

**Mirror gate.** A mirror clone runs clip-driven layers from Entry at its own phase but executes no drivers (`docs/runtime.md` §Parameters) — ungated, its heartbeat clips would drive the live `P12Hb*` tags out of phase and its relay senders would sit at rest on the live channels, poisoning the classifier in any world with a mirror. A driver-set `Alive` bool (the `mirror-detect` race) gates the whole `Rig` GameObject: real copies activate it within ~2 frames of load, a mirror clone never does.
...
| Receiver cells | ≤ 8 receivers per 5×5×5 broadphase cell, two sites 10 m apart | `docs/runtime.md` §Contacts carries the ~24-clustered-receiver misread bug; the 5×5×5 cell granularity and the per-cell bound are operator-measured, not that doc's. Receivers are `localOnly`, so only the observing client's own 8 exist per cell |
...
The prefab is hand-maintained against this section; clip bindings name these paths verbatim, so a rename silently unbinds the relay.
    ...
    │  ├─ Scan/Strobe/S+X        position-encoded strobe: sender rest z 0.5, +2.0 x Strobe;
    │  │                         Constant box (1,1,1) at z 2.5, tag P12Strobe
