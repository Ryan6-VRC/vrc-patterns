...
A tier is two states because an exit-time rung that also carries conditions re-checks them only when the state's time crosses the exit time again — every clip period when the exit time is 1 or less, never when it is above 1 — so one state holding the floor and exiting on release would quantize the release to the floor period (measured on a bare Animator).
...
| Roll range | the `max` of each React entry's Random driver (0..999) — **all three must agree**; the band widths below are fractions of it |
| Tier bands | the `Roll less N` rungs in each React state: 2 / 7 / 107 over 1000 = super rare 1 in 500, rare 1 in 200, uncommon 1 in 10, disjoint, rarest first — **all three ladders must agree** |
| Tier hold floor | each `zt_special<k>` clip's `seconds:` — the minimum the code stays set (0.3 s covers the 0.2 s wire floor with a margin down to ~20 fps); tier 3's 3 s is a lockout, so its clip *is* the reaction |

The width the wire sees is the clip plus or minus about two frames (the set and the clear each land at a state boundary, and the `Held` hop adds one): measured **0.333 s on the wearer and 0.317 s on a clone** against the 0.3 s clip at 60 fps, and 3.035 s / 2.95 s against the 3 s lockout.
