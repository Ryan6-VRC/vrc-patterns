...
| [`absolute-grip-prop`](absolute-grip-prop/) | A prop anyone can grab that lands in the same authored grip in every hand on every client, every time — no capture, so repeated pickups never drift apart across clients | 6dof-grab-prop's palm readout + a HandL/HandR gate pair for the hand + a FingerIndex cue pair for the axis sign, a confirm dwell that decides once, and an authored per-hand grip the rotor rides; snap-on tip; 1 synced bit, 12 receivers | Module |
...
| [`smooth-frametime`](smooth-frametime/) | Framerate-independent easing for jittery inputs (OSC hardware, remote grabs) — an exponential smoother plus a constant-velocity linear smoother that lands exactly, with the frame-rate floor its taper buys stated as a rule | frametime-aware AAP smoothers: two exponential α-flavours + a tapered constant-velocity linear smoother | Pattern, study |
