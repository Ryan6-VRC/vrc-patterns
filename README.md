...
| [`grip-sync`](compositions/grip-sync/) | A hand tool that lands in the same authored grip in every hand on every client, and rests exactly where it is set down for everyone, late joiners included: `absolute-grip-prop`'s carry under `grab-sync`'s arbitration, with a full-rotation word so the rest keeps the tilt the wrist left | `absolute-grip-prop` · `object-sync` (full rotation) · `word-channel`; 29 synced bits, 24 receivers |
