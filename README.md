...
| [`absolute-grip-prop`](absolute-grip-prop/) | A prop anyone can grab that lands in the same authored grip in every hand on every client, every time — orientation recovered from the grabber's own palm sender, no install on their side, and no capture, so repeated pickups never drift apart across clients | grab-prop's cell + 8 box receivers reading the built-in palm capsule into an oriented-pattern blend-tree readout + a VRC aim pair, a HandL/HandR gate pair for the hand, a FingerIndex cue pair for the axis sign, a confirm dwell that decides once, and two authored grips per hand the rotor rides, the nearer of the two to the prop's own feature taken by four hold-nearness receivers; snap-on tip; 3 synced bits, 16 receivers | Module |
...
| [`zone-touch`](zone-touch/) | Touch zones on the body that react when touched — a headpat, a poke. The common reaction fires instantly on every client (zero latency, no sync), while each touch rolls a rarity table whose tiered special — floored or a full lockout — is synced so everyone shows it in full | remote-firing receivers reproduce the common reaction per-client; only the rolled tier syncs (2 bits) under a hold floor; 3 bits with the enable | Module |
...
| [`smooth-frametime`](smooth-frametime/) | Framerate-independent easing for jittery inputs (OSC hardware, remote grabs) — an exponential smoother plus a constant-velocity linear smoother that lands exactly | frametime-aware AAP smoothers: two exponential α-flavours + a tapered constant-velocity linear smoother | Pattern, study |
...
| [`grip-sync`](compositions/grip-sync/) | A hand tool that lands in the same authored grip in every hand on every client, and rests exactly where it is set down for everyone, late joiners included: `absolute-grip-prop`'s carry under `grab-sync`'s arbitration, with a full-rotation word so the rest keeps the tilt the wrist left | `absolute-grip-prop` · `object-sync` (full rotation) · `word-channel`; 31 synced bits, 28 receivers |
