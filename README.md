...
| [`solve-order-pin`](solve-order-pin/) | Make a cyclic constraint rig behave the same way every session — for a sample-and-hold or capture-on-release cell that works on some play sessions and not others, with no edit between | 16 chained inactive constraints wired as a weight-0 source onto the constraint that must solve last, switched off after load; 0 synced bits | Module |
...
| [`object-sync`](object-sync/) | A prop set down anywhere in the world that is in the same place for everyone, late joiners included — millimetre placement over a ±8192 m range, no physbone anywhere | two-stage contact measure (coarse cell + redundant bias-cancelling fine) + aim-constraint rotation, over word-channel; 29 synced bits (a 28-bit wire + the enable), ~0.7 s refresh | Module |
