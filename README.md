...
| [`sync-probe`](sync-probe/) | Measure what the shipping client actually delivers of another player's synced parameters — schedule, loss, downshift, pauses, snapshot coherence — logged over your own OSC, no remote monitoring | rate-ladder counters + torn pair relayed off the remote copy via same-frame analog contacts; 49 synced bits, two wearers | Module |
