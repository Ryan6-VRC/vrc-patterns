...
| A **wearer-only** prop that rests at five anchors — chest, either hand, mouth, or frozen in the world — moved by a fist grip | five-anchor constraint multiplexer, gesture-release commit, chop-exempt mouth anchor, FreezeToWorld band; **1 synced int** | Module | [`anchor-prop`](anchor-prop/) |
| A payload — a marker or companion — that follows one point on another player: their hand or head | contact latching + trilateration cage; 1 synced bit | Module | [`contact-tracker`](contact-tracker/) |
| Same, reconstructed to an exact absolute position with unlimited range, on a smaller contact budget | 4-receiver linear reconstruction + crawler servo | Module | [`box-tracker`](box-tracker/) |
| Give a dragged or tracked object a **facing** (it turns the way it moves) | rotation from position history: physbone pull-cord + aim constraint; **0 synced params, no controller** | Structural Module | [`drag-bone`](drag-bone/) |
| Touch zones on the body that react when someone touches them — a headpat, a poke — with everyone seeing the reaction | remote-firing receivers reproduce the common reaction per-client; only the divergent outcome syncs; 2 bits | Module | [`zone-touch`](zone-touch/) |
...
| Bouncy ears, tails, and accessories that lag and spring — **without a PhysBone** | self-referencing constraints (the mechanical exponential smoother) | Structural Module | [`spring-damping`](spring-damping/) |
