...
| [`selective-animation`](selective-animation/) | An animation only the player you point at can see — you aim it by raising your arm, and no one else in the instance sees it | forearm-aimed `VRCRaycast` in `HitPlayers`: a player capsule exists only on its owner's client, so the hit lands on the target's machine alone; 0 synced bits | Structural Module |
