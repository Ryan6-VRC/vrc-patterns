...
- **A world-space `Target` is a different regime, and it needs a world-pinned parent.** The bullets above pick which *bone* the rig lags within; a `Target` that must stay put in the world offers no bone to pick. These rigs resist a moving parent without holding against it — the same one-pole lag, so the payload drags by roughly `21 ×` the parent's **per-frame** step (§Framerate dependence carries the law), which at walking pace is tens of centimetres. Parent the rig under a **world-pinned frame** instead — a `VRCParentConstraint` sourcing a never-instantiated prefab asset, or a source-less `FreezeToWorld` on a node of its own — and everything below it damps as if the avatar were standing still, to within one frame of parent motion and with no cumulative drift (measured). **Either idiom serves here**: this frame only has to be inertially static, and its own origin never reaches the output, because the payload converges on `Target` — already world-correct on every client — and not on the frame. So pick on cost. **Reuse a pin the rig already sits under** rather than minting one, and prefer that to a freeze, which costs a second component, a capture edge to arm, and a build-time gate to stop its irreversible capture firing while you author. A frame whose position is *consumed* instead of merely static is the other question, and `runtime.md` §Constraints is canon on it and on both idioms; this bullet is a declared echo of that section, sited here because the self-referencing constraint that makes the trap reachable is this entry's own.
...
**The number is `τ = 1/ln(1 + w_target/w_self)` frames**, the time constant of a one-pole lag, and it is what makes the weight table above retunable rather than magic: `PositionDamping`'s shipped 1 : 0.05 is τ = 20.50 frames, measured onto the predicted value exactly, settling within ~3τ. Two consequences worth having before you touch a weight. Halving `Target`'s weight does not double the settle — τ moves as the log — and the same τ sets the steady drag under a moving parent: about `21 ×` the per-frame parent step at this tuning (§Parent-transform dependency). A displacement that completes in far fewer frames than τ passes through nearly unattenuated, so a fast move looks undamped and a slow one looks heavily damped from one unchanged rig.

## Traps

**Do not set `FreezeToWorld` on the damper's own constraint.** Reaching for it is natural — the rig drags under a moving parent, and the bit is right there on the same component — and it kills the damping outright: the freeze and the damper both write that transform, the freeze wins, and the payload stops responding to `Target` altogether (measured: exactly zero response for 150 frames with the target half a metre away, no error logged). Moving the bit onto a second, source-less constraint over the same transform changes nothing. The freeze belongs on a **node of its own, above the rig** — which is the world-pinned parent §Parent-transform dependency asks for anyway, so the fix and the correct rig are the same thing.
