...
**The number is `τ = 1/ln(1 + w_target/w_self)` frames**, the time constant of a one-pole lag, and what makes the weight table retunable rather than magic: `PositionDamping`'s shipped 1 : 0.05 is τ = 20.50 frames, measured onto the predicted value exactly, settling within ~3τ. Halving `Target`'s weight does not double the settle — τ moves as the log — and the same τ sets the steady drag under a moving parent, about `21 ×` the per-frame parent step at this tuning (§Parent-transform dependency). A displacement that completes in far fewer frames than τ passes through nearly unattenuated, so one unchanged rig makes a fast move look undamped and a slow one heavily damped.

## Traps

**Do not set `FreezeToWorld` on the damper's own constraint.** The rig drags under a moving parent and the bit sits right there on the same component, so the reach is natural — and it kills the damping outright: freeze and damper both write that transform, the freeze wins, and the payload stops responding to `Target` altogether (measured: exactly zero response for 150 frames with the target half a metre away, no error logged). Moving the bit onto a second, source-less constraint over the same transform changes nothing. The freeze belongs on a **node of its own, above the rig** — the world-pinned parent §Parent-transform dependency asks for anyway, so the fix and the correct rig are the same thing.
